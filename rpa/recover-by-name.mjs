// Récupère l'étiquette d'un colis de l'espace client identifié par le texte de sa carte,
// vérifie un mot attendu, et la poste pour une réf. Usage: node recover-by-name.mjs <cardText> <ref> <mot>
import { chromium } from "playwright"
import { readFileSync } from "node:fs"
import { fileURLToPath } from "node:url"
import { dirname, join } from "node:path"
const __dirname = dirname(fileURLToPath(import.meta.url))
const [cardText, ref, expect] = process.argv.slice(2)
const env = {}
...
await page.goto("https://www.laposte.fr/espaceclient/", { waitUntil: "domcontentloaded" })
await page.locator("#onetrust-accept-btn-handler").click({ timeout: 2500 }).catch(() => {})
await page.waitForTimeout(4000)
await page.locator(`button:has-text("Colis - ${cardText}"), a:has-text("Colis - ${cardText}")`).first().click({ timeout: 8000 }).catch((e) => console.log("clic carte KO", e.message))
...
if (!ok) { console.log("⛔ PAS postée."); process.exit(1) }
